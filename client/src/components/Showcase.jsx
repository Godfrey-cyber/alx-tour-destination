	...
	return (
		<div className="flex w-full h-screen flex flex-col relative">
			...
			<div className="mt-5 flex space-x-6 items-center">
				<span
					onClick={prevProduct}
					className="absolute top-1/2 bottom-1/2 transform -translate-y-1/2 translate-x-1/2 left-2 z-50 flex justify-center items-center bg-white text-white rounded-full w-10 h-10 hover:bg-amber-400 transition-all delay-300 cursor-pointer"
				>
					<MdOutlineChevronLeft className="h-5 w-5 text-gray-500" />
				</span>
				<span
					onClick={nextProduct}
					className="absolute top-1/2 bottom-1/2 transform -translate-y-1/2 -translate-x-1/2 right-2 z-50 flex justify-center items-center bg-white text-white rounded-full w-10 h-10 hover:bg-amber-400 transition-all delay-300 cursor-pointer"
				>
			...
			<div className="mt-5 flex items-center space-x-2 absolute left-1/2 bottom-25 lg:bottom-18 transform -translate-x-1/2 z-50">
				{showcaseContent.map((_, index) => (
					<button
						...
						className={`rounded-full border border-amber-500 ${currentIndex === index ? 'bg-amber-500 w-3 h-3' : 'bg-gray-100 w-2 h-2'} transition-all delay-300`}
					></button>
				))}
			</div>
