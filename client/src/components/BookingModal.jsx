...
const BookingModal = ({ formData, handleDateValidation, setFormData }) => {
	...
	const handleSubmit = async event => {
		...
		if (
			composedData.destination &&
			composedData.startDate &&
			composedData.endDate &&
			composedData.children &&
			composedData.rooms &&
			composedData.adults
		) {
			try {
		...
		<div className="w-76 rounded-md h-fit min-h-48 shadow-lg shadow-gray-300 absolute top-25 right-0 z-50 bg-white">
			<div className="flex flex-col space-y-4 p-6 w-full">
				<div className="flex items-center justify-between space-x-3 w-full">
					...
					<span className="flex items-center space-x-4 border border-gray-400 rounded-md p-1 w-fit">
						<p
							...
							className="text-lg font-normal text-gray-500 cursor-pointer px-1 hover:bg-orange-500 rounded-md"
						>
							-
						</p>
						<p className="text-lg font-normal text-gray-500 px-1">
							{adultCount}
						</p>
						<p
							onClick={increaseAdult}
							className="text-lg font-normal text-gray-500 cursor-pointer hover:bg-orange-500 rounded-md px-1"
						>
							+
						</p>
					</span>
				...
				<button
					...
					type="submit"
					className="bg-amber-500 px-4 py-2 text-white rounded-md cursor-pointer"
				>
					Done
				</button>
			</div>
