				...
				{!destinations ? "No Destinations" : destinations?.map(data => (
					...
					>
						...
						<div className="flex flex-col space-y-3 p-4">
							<p className="text-sm text-gray-700 font-semibold">{data.title}</p>
							<span className="flex space-x-4 items-center justify-between">
								<p className="text-sm text-blue-600 font-semibold">{data.location.city}</p>
								<p className="text-xs text-blue-600 font-semibold">Show on Map</p>
							</span>
							<p className="text-xs text-gray-700 font-normal">9.3 Km from downtown</p>
							<p className="text-xs text-gray-700 font-normal">{data.description}</p>
						</div>
					</div>
				))}
			</div>
